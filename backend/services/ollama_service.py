...
class OllamaService(AIService):
    ...
    def __init__(self, config: AIServiceConfig):
        ...
        self.api_key = (
            self.ollama_config.api_key.get_secret_value()
            if self.ollama_config.api_key
            else None
        )
    ...
    @async_retry(
        ...
        retry_delay=1,
        backoff_factor=2,
        exceptions=(httpx.HTTPError, json.JSONDecodeError),
    )
    ...
    async def get_models(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: 模型列表，如果获取失败则返回空列表
        """
        ...
        try:
            ...
            logger.error(
                f"获取Ollama模型列表失败，HTTP状态码: {e.response.status_code}, 错误: {e}"
            )
            return []
        except httpx.RequestError as e:
            logger.error(f"获取Ollama模型列表请求失败: {e}")
            return []
        except json.JSONDecodeError as e:
            logger.error(f"解析Ollama模型列表响应失败: {e}")
            return []
        except Exception as e:
    ...
    async def test_connection(
        ...
        Args:
            model: 模型名称，如果提供则测试该模型是否可用

        Returns:
            Dict[str, Any]: 测试结果，包含success和message字段
        """
        try:
            # 首先测试API连接
            url = f"{self.base_url}/api/tags"
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if "models" not in data:
                    return {
                        "success": False,
                        "message": "Ollama服务响应格式不正确，缺少models字段",
                    }

                # 如果指定了模型，检查模型是否存在
                if model:
                    model_exists = any(
                        m.get("name") == model for m in data.get("models", [])
                    )
                    if not model_exists:
                        return {
                            "success": False,
                            "message": f"模型 {model} 不存在，请确保已在Ollama中拉取该模型",
                        }
            ...
            logger.error(
                f"测试Ollama连接失败，HTTP状态码: {e.response.status_code}, 错误: {e}"
            )
            return {
                "success": False,
                "message": f"Ollama服务返回错误: HTTP {e.response.status_code}",
            }
        except httpx.RequestError as e:
            logger.error(f"测试Ollama连接请求失败: {e}")
            return {
                "success": False,
            ...
            return {"success": False, "message": "Ollama服务响应格式不正确"}
        except Exception as e:
            logger.error(f"测试Ollama连接失败: {e}")
            return {
                "success": False,
                "message": f"测试Ollama连接失败: {str(e)}",
            }
